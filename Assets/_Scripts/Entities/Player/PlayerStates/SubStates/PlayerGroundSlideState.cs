...
public class PlayerGroundSlideState : PlayerGroundedState {
    ...
    public override void LogicUpdate() {
        ...
        if (!stopSlide && (((cumulatedGroundSlideTime >= playerData.groundSlideDuration || yInput == 1) && !player.isTouchingCeiling) || cumulatedGroundSlideTime >= playerData.groundSlideMaxDuration || player.CurrentVelocity.x.AbsoluteValue() == 0f || player.isTouchingWall)) {
            stopSlide = true;
        }

        if (stopSlide) {
            if (player.isTouchingCeiling || crouchInputHold || !player.CheckForSpace(player.GroundPoint.position.ToVector2() + Vector2.up * 0.015f, Vector2.up, 1.1f) || player.CheckHazards(player.MidPoint.position, Vector2.up, 1.1f)) {
                player.SetColliderParameters(player.MovementCollider, playerData.crouchColliderConfig, true);
                player.SetColliderParameters(player.HitboxTrigger, playerData.crouchColliderConfig);

                if ((player.CurrentVelocity.x.AbsoluteValue() == 0f || player.isTouchingWall) && xInput == 0f || player.FacingDirection == -player.CurrentVelocity.x.Sign()) {
